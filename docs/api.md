...
{
  "card_id": "...",
  "client_review_id": "uuid generated by client",
  "mode": "review",
  "rating": "good",
...
`client_review_id` 可选，但 iOS 客户端应始终传。后端按 `user_id + client_review_id` 幂等：同一个值重试不会重复增加 `review_count`，也不会重复写入 `review_events`。这是 Review 页乐观切换和本地 pending queue 的一致性基础。
